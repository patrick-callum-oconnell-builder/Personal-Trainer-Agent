...
async def test_create_workout_tracker(agent):
    ...
    try:
        spreadsheet = await agent_instance.tool_manager.sheets_service.create_workout_tracker("Workout Tracker")
...
async def test_add_workout_entry(agent):
    ...
    try:
        ...
        result = await agent_instance.tool_manager.sheets_service.add_workout_entry(
